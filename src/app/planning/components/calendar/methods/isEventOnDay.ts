...
const isEventOnDay = (event: Evenement, dayTimestamp: number): boolean => {
  ...
  return (
    (new Date(event.dateDebut).getTime() >= dayStart &&
      new Date(event.dateDebut).getTime() <= dayEnd) ||
    (new Date(event.dateFin).getTime() >= dayStart &&
      new Date(event.dateFin).getTime() <= dayEnd) ||
