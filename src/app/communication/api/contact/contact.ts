 ...
 */
type ResultatOperation<T> = { succes: boolean; message: string; contact: T | null };
 ...
 * @returns
 */
function resultOf<T>(succes: boolean, message: string, contact: T | null): ResultatOperation<T> {
...
export async function creerContact(contact: ContactInformation) {
  const verificationResultat = validerContact(contact);
...
export async function trouverContactParId(id: number) {
...
export async function obtenirTousLesContacts() {
