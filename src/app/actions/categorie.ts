...
export async function creerCategorie(datas: FormData) {
  ...
  try {
    if (!nom || !couleur || !compagnieId) {
      return {
        message: "Des informations sont manquantes, la catégorie ne peut pas être crée.",
        status: 400,
        categorie: null,
      };
    }
...
export async function getCategories() {
  try {
    const idCompagnie = 1;
    if (!idCompagnie) {
      return { message: "L'id de la compagnie n'est pas récupéré", status: 400, categories: null };
    }
